...
// Explore a single recipe
exports.exploreRecipe = async (req, res) => {
  const recipeId = req.params.id;

  try {
    const recipe = await Recipe.findById(recipeId);
    const isFavorite = recipe.favorites && recipe.favorites.includes(recipeId);
...
exports.addFavorite = async (req, res) => {
  ...
  try {
    ...
    // Simulate a single user's favorite list by adding recipe ID to the favorites array
...
exports.removeFavorite = async (req, res) => {
  ...
  try {
    ...
    if (recipe.favorites) {
      recipe.favorites = recipe.favorites.filter(id => id.toString() !== recipeId.toString());
      await recipe.save();
    }
...
exports.getAboutPage = (req, res) => {
  res.render('about', { title: 'About Us' });  // Render the 'about.ejs' template with a title
};
...
exports.viewFavorites = async (req, res) => {
  try {
    const recipes = await Recipe.find({ favorites: { $exists: true, $ne: [] } });
